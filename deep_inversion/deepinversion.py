...
from .utils import lr_cosine_policy, clip, denormalize, InversionSampler
...
class DeepInversionFeatureHook(object):
    ...
    def __init__(self, module):
        self.r_feature = None
        self.hook = module.register_forward_post_hook(self.hook_fn)

    def hook_fn(self, module, inputs, output):
        # hook co compute deepinversion's feature distribution regularization
        nch = inputs[0].shape[1]
        mean = inputs[0].mean([0, 2, 3])
        var = inputs[0].transpose([1, 0, 2, 3]).reshape([nch, -1]).var(1, unbiased=False)

        # forcing mean and variance to match between two distributions
        # other ways might work better, i.g. KL divergence
        r_feature = paddle.norm(module._variance - var, 2) + paddle.norm(
            module._mean - mean, 2)
...
class DeepInversionFeatureHook1D(object):
    ...
    def hook_fn(self, module, inputs, output):
        ...
        var = inputs[0].transpose([1, 0]).reshape([nch, -1]).var(1, unbiased=False)
...
class DeepInversionClass(object):
    def __init__(self, bs=64,
        ...
        self.bn_mean = self.net_teacher.feature_extractor.bn._mean
        self.bn_var = self.net_teacher.feature_extractor.bn._variance
    ...
    def get_images(self, targets=None, preprocessor=None, starts=None):
        ...
        inputs = paddle.randn((targets.shape[0], 3, 256, 128)) if starts is None else paddle.to_tensor(starts(),
                                                                                                       stop_gradient=False)

        pooling_function = nn.AvgPool2D(kernel_size=2)
        optimizer = optim.Adam(parameters=[inputs], learning_rate=self.lr, beta1=0.5, beta2=0.9, epsilon=1e-8)
        ...
        for iterations_per_layer, downsampling_ratio in zip([2000, 1000], [2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                self.net_teacher.clear_gradients()
                ...
                acc = paddle.mean((paddle.argmax(preds, axis=1) == targets).astype(paddle.float32))
                ...
                inputs.set_value(clip(inputs.numpy()))

                if best_cost > loss.item() and downsampling_ratio == 1:
                    best_inputs = inputs.numpy()
                    best_cost = loss.item()
    ...
    def register_hooks(self):
        if not self.loss_r_feature_layers:
            for module in self.net_teacher.sublayers():
    ...
    def augment(inputs, preprocessor=None):
        ...
        if flip:
            inputs_jit = paddle.flip(inputs, axis=(3,))
        else:
            inputs_jit = inputs
    ...
    def image_loss(self, inputs):
        ...
        # l2 loss on images
        loss_l2 = paddle.norm(inputs.reshape([self.bs, -1]), axis=1).mean()
    ...
    def adi_loss(self, inputs, preds_teacher):
        ...
        preds_student = preds_student[..., -preds_teacher.shape[1]:]

        preds_student = preds_student.softmax(axis=1)
        # entropy = (-preds_student * preds_student.log()).sum(axis=1).mean()
...
class DeepInversionClassBeta(DeepInversionClass):
    ...
    def get_images(self, targets=None, preprocessor=None, starts=None):
        self.register_hooks()
        self.net_teacher.stop_gradient = False
        best_cost = np.inf
        ...
        pooling_function = nn.AvgPool2D(kernel_size=2)
        optimizer = optim.Adam(learning_rate=self.lr, beta1=0.5, beta2=0.9, epsilon=1e-08, parameters=[inputs])
        num_batch = targets.shape[0] // self.bs
        ...
        for iterations_per_layer, downsampling_ratio in zip([2000 * num_batch, 1000 * num_batch], [2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                # indices = paddle.randint(targets.shape[0], (self.bs, ))
                ...
                acc = paddle.mean((paddle.argmax(preds, axis=1) == batch_targets))
...
class DeepInversionClassMountain(DeepInversionClass):
    ...
    def get_images(self, targets=None, preprocessor=None, starts=None):
        ...
        if starts is None:
            inputs = paddle.randn((targets.shape[0], 3, 256, 128))
            inputs.stop_gradient = False
        else:
            inputs = paddle.to_tensor(starts(), stop_gradient=False)
        ...
        for iterations_per_layer, downsampling_ratio in zip([2000, 1000], [2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                acc = paddle.mean((paddle.argmax(preds, axis=1) == batch_targets).astype(paddle.float32))
    ...
    def mountain_loss(self, embeddings_t, embeddings_s, targets):
        ...
        for embedding_t, embedding_s, target in zip(embeddings_t, embeddings_s, targets):
            ...
            residual_t = predecessors_t - embedding_t.unsqueeze(axis=0)
            predecessors_s = self.predecessor_student[int(target)]
            residual_s = predecessors_s - embedding_s.unsqueeze(axis=0)
            residual_cosines = (F.normalize(residual_t, axis=1) * F.normalize(residual_s, axis=1)).sum(axis=1)
            losses.append(residual_cosines.mean())
            self_cosine = (F.normalize(embedding_t, axis=0) * F.normalize(embedding_s, axis=0)).sum()
...
class DeepInversionClassInc(DeepInversionClass):
    ...
    def get_images(self, targets=None, preprocessor=None, starts=None):
        self.register_hooks()
        self.net_teacher.stop_gradient = False
        self.net_inc.stop_gradient = False
        ...
        for iterations_per_layer, downsampling_ratio in zip([2000, 1000], [2, 1]):
            ...
            for iteration_loc in range(iterations_per_layer):
                ...
                optimizer.clear_grad()
                self.net_teacher.clear_gradients()
                self.net_inc.clear_grad()
                self.net_student.clear_grad()
    ...
    def adi_loss(self, inputs, preds_teacher):
        preds_student, outputs_student = self.net_student(inputs)
        preds_student = self.network_output_function(preds_student)
        preds_student_base = preds_student[..., -preds_teacher.shape[1]:]
        preds_student_base = preds_student_base.softmax(axis=1)
        preds_student_inc = preds_student[..., :-preds_teacher.shape[1]]
        preds_student_inc = preds_student_inc.softmax(axis=1)

        entropy = (-preds_student_base * preds_student_base.log()).sum(axis=1).mean()
        loss_verifier_cig = -entropy

        preds_inc, outputs_inc = self.net_inc(inputs)
        preds_inc = self.network_output_function(preds_inc)
        preds_inc = preds_inc[..., :-preds_teacher.shape[1]]
        preds_inc = preds_inc.softmax(axis=1)
