...
from ppcls.arch.backbone.modules import SeparateBN, Sequential

kaiming_normal_ = nn.initializer.KaimingNormal()

class SeparableResNet(nn.Layer):
    ...
    def __init__(self, pretrained=True, last_stride=2, last_pooling='avg', embedding=256, num_domains=2):
        ...
        bn.weight.set_value(paddle.ones_like(bn.weight))
        bn.bias.set_value(paddle.zeros_like(bn.bias))
    ...
    def forward(self, x, domain_indices=0, output_feature=None):
        ...
        if not self.training:
            feature = F.normalize(feature, 2, axis=1).reshape([B, -1])
...
class Base(nn.Layer):
    ...
    def _preprocess_state_dict(self, state_dict):
        ...
        for key in state_dict.keys():
            ...
            # note that: all the second sublayers in downsample blocks are BN layers.
