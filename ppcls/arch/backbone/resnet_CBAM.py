...
class ResNetCBAM(nn.Layer):

    def __init__(self, pretrained=True, last_stride=2, last_pooling='avg', embedding=256):
    ...
    def __repr__(self):
        return f"Build ResNet(pretrained={self.pretrained}, last_stride={self.last_stride}, " \
...
class Base(nn.Layer):
    ...
    def __init__(self, block, layers, zero_init_residual=False, groups=1, width_per_group=64,
                 replace_stride_with_dilation=None, norm_layer=None, last_stride=2, pretrained=False):
        ...
        if zero_init_residual:
            for m in self.sublayers():
                ...
                m.bn3.weight.set_value(paddle.zeros_like(m.bn3.weight))

        if pretrained:
            assert osp.isfile(
                osp.expanduser('~/.cache/paddle/checkpoints/resnet50-19c8e357.pth')), "paddle pretrained arch " \
                                                                                     "doesn't exists."
            state_dict = paddle.load(osp.expanduser('~/.cache/paddle/checkpoints/resnet50-19c8e357.pth'))
...
if __name__ == '__main__':
    model = Base(Bottleneck, [3, 4, 6, 3], pretrained=True)
    pass
