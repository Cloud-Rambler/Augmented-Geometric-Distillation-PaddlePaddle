...
from ppcls.evaluation import mean_ap, map_cmc
from ppcls.utils.meters import AverageMeter, AverageMeters
from ppcls.loss import euclidean_dist
from ppcls.loss.utils import cos


def extract_cnn_feature(model, inputs, output_feature='embedding', **kwargs):
    model.eval()
    outputs_origin = model(inputs, **kwargs)[output_feature]
    # outputs_flip = arch(inputs.flip(dims=[3]), output_feature=output_feature, **kwargs)
    # return [(output_origin + output_flip) / 2 for output_origin, output_flip in zip(outputs_origin, outputs_flip)]
    return outputs_origin


def extract_features(model, data_loader, output_feature="embedding", print_freq=1,  **kwargs):
    model.eval()
    ...
    with paddle.no_grad():
        for i, (imgs, fnames, pids, *_) in enumerate(data_loader):
            ...
            outputs = extract_cnn_feature(model, imgs, output_feature, **kwargs).detach()
...
def paddle_intersect(tensor1, tensor2):
    tensor1 = tensor1.unique()
    tensor2 = tensor2.unique()
    return paddle.to_tensor(np.intersect1d(tensor1.numpy(), tensor2.numpy()))

def re_ranking_dist(probFea, galFea, k1, k2, lambda_value, local_distmat=None, only_local=False): # 59.11451840400696
    # if feature vector is numpy, you should use 'paddle.to_tensor' transform it to tensor
    ...
    if only_local:
        ...
        # original_dist = np.tile(np.power(feat,2).sum(axis=1, keepdims=True), [1,all_num]) + \
        #          np.tile(np.power(feat, 2).sum(axis=1, keepdims=True), [1, all_num]).T
        original_dist = paddle.pow(feat, 2).sum(axis=1, keepdim=True)+ \
                        paddle.pow(feat, 2).sum(axis=1, keepdim=True).t()
        original_dist = paddle.addmm(original_dist, feat, feat.t(), 1, -2)
        del feat
        if not local_distmat is None:
            original_dist = original_dist + local_distmat
    gallery_num = original_dist.shape[0]
    original_dist = (original_dist / paddle.max(original_dist, axis=0)).t()
    V = np.zeros(original_dist.shape ,dtype=np.float32)
    initial_rank = paddle.argsort(original_dist).numpy()
...
class Evaluator(object):
    ...
    def evaluate(self, query_loader, gallery_loader, query, gallery, output_feature='embedding', re_ranking=False,
                 dist_func=pairwise_distance, dist_type='cos', print_freq=1, step=1024, cmc_topk=(1, 5, 10, 20), **kwargs):
        query_features = extract_features(self.model, query_loader, output_feature, print_freq=print_freq, **kwargs)
        print('Finish extracting feature of queries.')
        gallery_features = extract_features(self.model, gallery_loader, output_feature, print_freq=print_freq, **kwargs)
    ...
    def step_evaluate(query_features, gallery_features, query, gallery,
                      step, dist_func=pairwise_distance, dist_type='cos', re_ranking=False):
...
class IncrementalEvaluator(Evaluator):
    ...
    def evaluate(self, query_loaders, gallery_loader, queries, gallery, output_feature='embedding', re_ranking=False,
                 dist_func=pairwise_distance, dist_type='cos', print_freq=1, step=1024, cmc_topk=(1, 5, 10, 20), **kwargs):
        queries_features = {name: extract_features(self.model, query_loader, output_feature, print_freq=print_freq, **kwargs)
