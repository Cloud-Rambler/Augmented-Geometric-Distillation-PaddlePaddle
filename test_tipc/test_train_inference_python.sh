    ...
    for use_gpu in ${use_gpu_list[*]}; do
        ...
        # gpu
            ...
            for batch_size in ${batch_size_list[*]}; do
                ...
                set_batchsize=$(func_set_params "${batch_size_key}" "${batch_size}")
                set_model_dir=$(func_set_params "${infer_model_key}" "${_model_dir}")
                command="python main_incrementalX.py --dataset market --previous ./logs/msmt17 --logs-dir ./logs/msmt17-market_XsimGD --inversion-dir ./data/lite_data/generations_r50_msmt17 -g 0 --evaluate 30 --seed 1 --peers 2 --algo-config ./configs/inverXion.yaml"
                eval $command
                last_status=${PIPESTATUS[0]}
                eval "cat ${_save_log_path}"
                status_check $last_status "${command}" "${status_log}"  "${model_name}"
    ...
    for gpu in ${gpu_list[*]}; do
        ...
        for trainer in ${trainer_list[*]}; do
            ...
            if [ ${#gpu} -le 2 ];then  # train with single gpu
                cmd="python ./tools/main_incrementalX.py --dataset market --previous ./logs/msmt17 --logs-dir ./logs/msmt17-market_XsimGD --inversion-dir ./data/lite_data/generations_r50_msmt17 -g 0 --evaluate 30 --seed 1 --peers 2 --algo-config ./configs/inverXion.yaml "
            ...
            if [ ${eval_py} != "null" ]; then
                ...
                eval ${env}
                eval_cmd="python ./tools/evaluate.py --dataset msmt17 market --ckpt ./logs/msmt17-market_XsimGD/checkpoint.pdparams --output  "
                eval $eval_cmd
