...
from ppcls.utils.data.dataset import Dataset, MixedDataset
from ppcls.utils.data import build_test_loader
from ppcls.arch.backbone import ResNet
from ppcls.evaluation.evaluators import Evaluator, IncrementalEvaluator
from ppcls.utils.serialization import load_checkpoint
import sys
...
def main(args):
    ...
    if args.recursive:
        ...
        ckpts = glob.glob(osp.join(args.ckpt, '**/checkpoint*.pdparams'), recursive=True)
    else:
        if osp.isdir(args.ckpt):
            ckpts = [osp.join(args.ckpt, 'checkpoint.pdparams')]
    ...
    for ckpt in ckpts:
        ...
        model.set_state_dict(checkpoint["backbone"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", nargs='*', type=str, default=["market", "msmt17"])
    parser.add_argument("--height", type=int, default=256)
    parser.add_argument("--width", type=int, default=128)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("-c", "--ckpt", type=str, default='./logs/msmt17-market_XsimGD/checkpoint.pdparams')
    parser.add_argument("-r", "--re-ranking", action='store_true')
    parser.add_argument("-o", '--output', action='store_true', default=True)
