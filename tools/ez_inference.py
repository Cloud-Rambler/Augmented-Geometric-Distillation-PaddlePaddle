...
from ppcls.arch import Linear, ResNet
from ppcls.utils.data import build_train_loader
...
def main(args):
    ...
    classifier_old = Linear(args.embedding, num_previous_classes)
    manager = CheckpointManager(model=model_old, classifier=classifier_old)
    manager.load(checkpoint)

    model_old = paddle.DataParallel(model_old)
    model_old.eval()
    classifier_old = classifier_old
    classifier_old.eval()
    ...
    for inputs in training_loader_old:
        ...
        print(paddle.mean((paddle.argmax(preds, axis=1) == pids)))
